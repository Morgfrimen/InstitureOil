...
void RunTaskWater()
{
	...
	var nastyaTask = CoreTask.GeCoreTask().GeWaterTaskUintValue(list);
...
void RunTaskNastya()
{
	...
	var nastyaTask = CoreTask.GeCoreTask().GetNastyaTask(n, list);
	Console.WriteLine($"Результат: {nastyaTask.Run()}");
}
...
void RunTaskBear()
{
	...
	var bear = CoreTask.GeCoreTask().GetBearTask(n, k, list);
